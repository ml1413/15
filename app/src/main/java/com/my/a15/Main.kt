...
import com.my.a15.FifteenEngine.Companion.myPrint


fun main() {
    val engine = FifteenEngine
    var state = engine.getInitialState()
    while (engine.isWin(state)) {
        myPrint(state)
        println()
        println("Введите число с таблицы -> ")
        val readLine = readln().toIntOrNull() ?: -1
        state = engine.transitionState(oldState = state, cell = readLine)
    }
}
